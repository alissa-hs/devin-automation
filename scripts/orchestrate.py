...
def create_devin_session():
    ...
    - The PR title should reference the issue title
    """
    print(f"Creating Devin session for issue #{ISSUE_NUMBER}: {ISSUE_TITLE}")
    print(f"Calling Devin API at {DEVIN_BASE}/sessions")
    ...
    print(f"Devin API response body: {response.text}")
...
def post_github_comment(status, session_id, session_url):
    if status == "success":
        ...
        **Devin Session:** {session_url}
        **Session ID:** `{session_id}`
        Devin was unable to complete this fix. Please review the session logs for more details.
        """
